 ...
 * @param config
 */
export function resolveInlineIcon(name = '', config = {}) {
    if (!name) {
        throw new Error('the parameter `name` is required to resolve the inline icon');
    }

    let source = iconsSource;
    if (config?.source) {
